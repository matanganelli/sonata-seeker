...
const generateMockAnalysis = (duration: number): SonataAnalysis => {
  ...
  return {
    ...
    musicalInsights: ['Forma sonata típica do período clássico', 'Relação tônica-dominante na exposição', 'Desenvolvimento com fragmentação temática', 'Recapitulação mantém ambos os temas na tônica']
  };
};
const Index = () => {
  const {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/analyze-sonata`, {
        method: 'POST',
        headers: {
  ...
  return <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-12">
        ...
        y: 0
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Identifique a estrutura da forma sonata em arquivos MIDI 
          </p>
        </motion.header>

        {/* Upload section */}
        <section className="mb-12">
          <MidiUploader onFileUpload={handleFileUpload} isLoading={isParsing || isAnalyzing} />
        ...
        y: 0
            ...
            <div>
              ...
              <PianoRoll midiData={midiData} sections={analysis?.sections} />
            </div>
        ...
        {!midiData && <motion.section initial={{
        opacity: 0
      }} animate={{
        opacity: 1
      }} transition={{
      ...
      <footer className="absolute bottom-1 right-8 text-right z-20">
        ...
        <a href="https://www.mariatanganelli.com" target="_blank" rel="noopener noreferrer" className="text-xs text-yellow-400 hover:text-yellow-300 hover:underline transition-colors mt-2 block cursor-pointer bg-transparent">
          mariatanganelli.com
        </a>
      </footer>
    </div>;
